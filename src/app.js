...
let charcoil1 = document.getElementById("charcoil1");
let charcoil2 = document.getElementById("charcoil2");
...
function updateAbility(ability) {
    ...
    if (ability1 == "Natural Calamity" || ability2 == "Natural Calamity") calamity.checked = true;
    else noEffect.checked = true;
...
$(".moveSelect").change(function() {
    ...
    if (move.hits) {
        ...
        if (move.name == "Ferocious Onslaught") {
            moveGroupObj.children(".move-hits").children(".fero").hide();
        } else if (move.name !=  "Ferocious Onslaught") {
            moveGroupObj.children(".move-hits").children(".fero").show();
        }
...
function loadStats() {
    ...
    rawStats1 = {atk: 0, def: 0, atkR: 0, defR: 0, spd: 0};
    rawStats2 = {atk: 0, def: 0, atkR: 0, defR: 0, spd: 0};
    ...
    rawStats1.atk = atk1;
    rawStats1.def = def1;
    rawStats1.atkR = atkR1;
    rawStats1.defR = defR1;
    rawStats1.spd = spd1;
    ...
    rawStats2.atk = atk2;
    rawStats2.def = def2;
    rawStats2.atkR = atkR2;
    rawStats2.defR = defR2;
    rawStats2.spd = spd2;
    ...
    let turnRemainder1 = (repeating1.value > 0 ? (repeating1.value & 1 == 1 ? "Odd" : "Even") : "None");
    trueStats1.atk = atk1;
    ...
    let turnRemainder2 = (repeating2.value > 0 ? (repeating2.value & 1 == 1 ? "Odd" : "Even") : "None");
    trueStats2.atk = atk2;
...
function battleAdjustments(move, ability1, ability2, stuffUsed, atk, def, boastAttack, tempType, abilityCheck1, abilityCheck2, crit, stat2, firstHit, hitConfirm, foulHit, stats1, stats2) {
    ...
    if ((move.stat && move.stat.battle == "Offense" && move.stat.stat == atk.name && !(move.secondaryEffect && ability1 == "Brute Force")) || (ability2 == "Guilt" && atk.name == "AttackM") || (move.name == "Chaotic Bolt" && stat2 == "frozen") || (move.name == "Icefall" && parseInt(stats1.spd) < parseInt(stats2.spd))/* || (move.name == "Gleamspire" && celebrate.checked) || (ability2 == "Signal Jammer" && atk.name == "AttackR")*/ && !(ability2 == "Sugarsick" && atk.name == "AttackR")) {
        ...
        if (move.name == "Chaotic Bolt" && stat2 == "frozen") moveMod += (ability1 == "Opposite Day" ? -1 : 1); 
        else if (move.stat) moveMod += (ability1 == "Opposite Day" ? move.stat.amount * -1 : move.stat.amount);
        else if (move.name == "Icefall" && parseInt(stats1.spd) < parseInt(stats2.spd)) moveMod += (ability1 == "Opposite Day" ? -1 : 1);
        //else if (move.name == "Gleamspire" && celebrate.checked) moveMod += (ability1 == "Opposite Day" ? 0 : 1);
    ...
    //Sugarsick
    if (ability2 == "Sugarsick" && atk.name == "AttackR" && move.mr1 == "Ranged Attack") {
        moveMod = 0;
        let sugarCheck = 0;
        if (abilityCheck2) {
            sugarCheck += (ability1 == "Opposite Day" ? 1 : -1);
            if (celebrate.checked && sugarCheck < 0) sugarCheck = 0;
            if (ability1 == "Resolve" && move.mr1 == "Ranged Attack" && sugarCheck < 0) sugarCheck += 2;
            else if (ability1 == "Stonefaced" || ability1 == "Evergreen Halo") sugarCheck = 0;
        }    
        moveMod += (ability1 == "Opposite Day" ? 1 : -1);

        if (ability1 == "Resolve" && move.mr1 == "Ranged Attack") {
            moveMod += 2;
            stuffUsed.ability1 = ability1;
        }

        if (move.name == "Chaotic Bolt" && stat2 == "frozen") moveMod += (ability1 == "Opposite Day" ? -1 : 1); 
        else if (move.stat && move.stat.battle == "Offense") moveMod += (ability1 == "Opposite Day" ? move.stat.amount * -1 : move.stat.amount);

        if (celebrate.checked && moveMod < 0) moveMod = 0;
        if (ability1 == "Stonefaced" || ability1 == "Evergreen Halo") {
            moveMod = 0;
            stuffUsed.ability1 = ability1;
        }
        atkStage = (moveMod < 0 ? Math.max(atkStage + sugarCheck + moveAdjustmentCount * moveMod, -6) : Math.min(atkStage + sugarCheck + moveAdjustmentCount * moveMod, 6));
    ...
    if ((ability2 == "Menacing Snarl" || ability2 == "Sickly Sweet") && abilityCheck2) {
        moveMod = 0;
        ...
        if (ability1 == "Stonefaced" || ability1 == "Opposite Day" || ability1 == "Evergreen Halo") {
            moveMod = (ability1 == "Opposite Day" ? 1 : 0);
            stuffUsed.ability1 = ability1;
        }
    ...
    if ((move.stat && move.stat.battle == "Defense" && move.stat.stat == def.name && !(move.secondaryEffect && ability1 == "Brute Force")) || (move.name == "Chaotic Bolt" && stat2 == "vulnerable")) {
        moveMod = 0;
        if (move.name == "Chaotic Bolt" && stat2 == "vulnerable") moveMod += (ability2 == "Opposite Day" ? 1 : -1);
        else if (move.stat) moveMod += (ability2 == "Opposite Day" ? move.stat.amount * -1 : move.stat.amount);
    ...
    if ((((ability2 == "Strong Armor" && move.contact) || (ability2 == "Floaty" && tempType == "Air")) && move.mr2 == "Melee Defense" && def.name == "DefenseM") || (ability2 == "Grand Entrance" && abilityCheck2) || (ability2 == "Cursed Growth")) {
        defStage = (ability2 == "Grand Entrance" ? Math.min(defStage + 1, 6) : Math.min(defStage + adjustmentCount, 6));
        def.def = (defStage < 0 ? Math.floor(baseDefense * (2 / (2 - defStage))) : Math.floor(baseDefense * ((2 + defStage) / 2)));
...
function detailedReport() {
    ...
    if (move.name == "Fatal Flaw" && stats2.defR > stats2.def) {
        ...
        adaptiveResult = "melee";
        atkDef = getTempAtkDef(second, adaptive, abilitys, ability);
    } else if ((move.name == "Talon Strike") && gen == "Male") {
        adaptive.mr = "Magic";
        adaptive.mr1 = "Ranged Attack";
        adaptive.mr2 = "Ranged Defense";
        adaptiveResult = "magical";
        atkDef = getTempAtkDef(second, adaptive, abilitys, ability);
    } if (abilitys == "Mixed Bag") {
        if (stats1.atk > stats1.atkR) {
            adaptive.mr = (adaptive.mr ? adaptive.mr : move.mr);
            adaptive.mr1 = "Melee Attack";
            adaptive.mr2 = (adaptive.mr2 ? adaptive.mr2 : move.mr2);
            adaptiveResult = "melee";
        } else if (stats1.atkR > stats1.atk) {
            adaptive.mr = (adaptive.mr ? adaptive.mr : move.mr);
            adaptive.mr1 = "Ranged Attack";
            adaptive.mr2 = (adaptive.mr2 ? adaptive.mr2 : move.mr2);
        } else {
            adaptive.mr = (adaptive.mr ? adaptive.mr : move.mr);
            adaptive.mr1 = (adaptive.mr1 ? adaptive.mr1 : move.mr1);
            adaptive.mr2 = (adaptive.mr2 ? adaptive.mr2 : move.mr2);
        } atkDef = getTempAtkDef(second, adaptive, abilitys, ability);
    } if (!atkDef) atkDef = getTempAtkDef(second, move, abilitys, ability);
    if ((move.mr == "Melee" && myStatus == "burned" && !firstLoom.types.includes("Fire") && !(adaptiveResult && adaptiveResult == "ranged")) || (move.mr == "Magic" && myStatus == "cursed" && !(adaptiveResult && adaptiveResult == "melee")) || (myStatus == "frozen" && icyFloor.checked && !firstLoom.types.includes("Ice"))) statStr = " " + myStatus.charAt(0).toUpperCase() + myStatus.slice(1);
    ...
    //tempAtk
    if ((move.mr1 == "Ranged Attack" && adaptiveResult != "melee") || (abilitys == "Mixed Bag" && stats1.atkR > stats1.atk) || (move.name == "Talon Strike" && adaptiveResult == "magical")) {
    ...
    else if ((move.mr1 == "Melee Attack" && !(abilitys == "Sand Swap" && sandstorm.checked)) || ((move.name == "Spectral Ire" || move.name == "Transcendent Power" || move.name == "Lilypad Leap") && adaptiveResult == "melee") || (abilitys == "Mixed Bag" && stats1.atk > stats1.atkR)) {
    ...
    else if ((move.mr2 == "Melee Defense" && adaptiveResult != "magical") || (move.name == "Fatal Flaw" && stats2.defR > stats2.def) || ((move.name == "Transcendent Power" || move.name == "Lilypad Leap") && adaptiveResult == "melee")) {
    ...
    else if (move.mr2 == "Ranged Defense" || (move.name == "Adaptive Assault" && firstLoom.baseStats.attackR > firstLoom.baseStats.attack) || (move.name == "Talon Strike" && adaptiveResult == "magical")) {
...
function getMultiplier(loom1, loom2, move, movePower, crit, repeat, hits, elemental, swarm, snowball, hitConfirmer = false, level, ul = false, second = false, detailed = false, withoutSlapDown = true, takeSecondaryType = false, foulHit = false, shardSurge = false, pylons = false) {
    ...
    if (second) {
        ...
        normieStats1 = rawStats2;
        normieStats2 = rawStats1;
    } else {
        ...
        normieStats1 = rawStats1;
        normieStats2 = rawStats2;
    }
    ...
    if (move.name == "Psychic Mirror" && !withoutSlapDown) {
        types1.primary = types2.primary;
        types1.secondary = types2.secondary;
    }
    ...
    if (move.name == "Fatal Flaw") {
        ...
        tempStats = getTempAtkDef(second, adaptive, ability1, ability2);
    } else if (move.name == "Talon Strike") {
        if (gen1 == "Male") {
            adaptive.mr = "Magic";
            adaptive.mr1 = "Ranged Attack";
            adaptive.mr2 = "Ranged Defense";
        ...
        tempStats = getTempAtkDef(second, adaptive, ability1, ability2);
        tempType = "Poison";
    } if (ability1 == "Mixed Bag") {
        if (theStats1.atk > theStats1.atkR) {
            adaptive.mr = (adaptive.mr ? adaptive.mr : move.mr);
            adaptive.mr1 = "Melee Attack";
            adaptive.mr2 = (adaptive.mr2 ? adaptive.mr2 : move.mr2);
        } else if (theStats1.atkR > theStats1.atk) {
            adaptive.mr = (adaptive.mr ? adaptive.mr : move.mr);
            adaptive.mr1 = "Ranged Attack";
            adaptive.mr2 = (adaptive.mr2 ? adaptive.mr2 : move.mr2);
        } else {
        ...
        stuffUsed.ability1 = ability1;
        tempStats = getTempAtkDef(second, adaptive, ability1, ability2);
    } if (!tempStats) tempStats = getTempAtkDef(second, move, ability1, ability2);
    ...
    if (ability1 == "Foul Welcome" && immuneBoostCheck1) {
        itemB = "None";
        stuffUsed.ability1 = ability1;
    }
    if (ability2 == "Foul Welcome" && immuneBoostCheck2) {
        itemA = "None";
    }
    ...
    if (ability1 == "Toxicity" && tempType == "Basic") {
        tempType = "Poison";
        multi *= 1.3;
        stuffUsed.ability1 = ability1;
    }
    ...
    if ((((move.name == "Retribution" || move.name == "Counter Punch") && parseInt(stats1.spd) < parseInt(stats2.spd) && !btl1)) ||
    ...
    if ((ability1 == "Envy" && itemA == "None") || 
       ...
       (ability1 == "Umbral Hunger" && (tempType == "Dark" || tempType == "Crystal") && stat2 != "healthy") ||
       (ability1 == "The Fungus" && move.drain) || 
       (ability1 == "Paper Cut" && stat2 == "vulnerable")) {
        multi *= 1.3;
        stuffUsed.ability1 = ability1;
    }
    ...
    if ((ability1 == "Destructive Anger" && stat1 == "enraged") ||
       ...
       ((ability1 == "Corrosion" || ability1 == "Decay") && tempType == "Poison" && immuneBoostCheck1)) {
        multi *= 1.5;
        stuffUsed.ability1 = ability1;
    }
    ...
    if ((ability1 == "Covetous" && itemB != "None") ||
       ...
       (ability1 == "Grounded" && tempType == "Earth") ||
       (ability1 == "Skyborn" && tempType == "Air")) {
        multi *= 1.25;
        stuffUsed.ability1 = ability1;
    }

    if ((ability1 == "Chlorobite" && move.bite) ||
        (ability1 == "Sky Deity" && tempType == "Air") ||
        (ability1 == "Extinguisher" && tempType == "Water")) {
        multi *= 1.33;
        stuffUsed.ability1 = ability1;
    }
    ...
    if ((ability1 == "Rapier" && move.priority) ||
       (ability1 == "Goliath") ||
       (ability1 == "King's Edict" && stats1.hpPercent >= 50) ||
       (ability1 == "Impersonate") ||
       (ability1 == "Fixation" && immuneBoostCheck1)) {
        multi *= 1.2;
        stuffUsed.ability1 = ability1;
    }
    ...
    if ((itemA == "Surge DNA" && loom1.name == "Velocivult") ||
       ...
       (itemA == "Virus DNA" && loom1.name == "Myphoxic")) {
        multi *= 1.3;
        stuffUsed.item1 = itemA;
    }
    ...
    if (rain.checked) {
        ...
        if (tempType == "Water" || tempType == "Fire" || loom2.types.includes("Ice") || (ability1 == "Flow State" && (move.mr1 == "Ranged Attack" || move.mr1 == "Speed"))) stuffUsed.weather += " in Rain";
    }
    ...
    if (sandstorm.checked) {
        if (tempType == "Earth") multi *= 1.25;
        if (tempType == "Earth" || loom2.types.includes("Crystal")) stuffUsed.weather += " in Sandstorm";
    }
    if (garden.checked && tempType == "Plant") {
        multi *= 1.25;
        stuffUsed.weather += " in the Garden";
    }
    if (celebrate.checked && (tempType == "Light" || move.name == "Gleamspire")) {
        if (tempType == "Light") multi *= 1.25;
        stuffUsed.weather += " during a celebration";
    }
    if (calamity.checked && tempType == "Fire") {
        multi *= 1.25;
        stuffUsed.weather += " in Natural Calamity";
    }
    ...
    if (icyFloor.checked && tempType == "Ice") {
        multi *= 1.25;
        stuffUsed.weather += " on an Icy Floor";
    }
    ...
    if (ability2 == "Signal Jammer" && move.mr == "Magic") {
        multi *= 0.8;
        stuffUsed.ability2 = ability2;
    }
    if ((move.mr1 == "Melee Attack" && ability1 == "Hidden Strength") ||
        (move.mr1 == "Ranged Attack" && ability1 == "Dishonest")) {
        multi *= 2;
        stuffUsed.ability1 = ability1
    }
    ...
    if (ability2 == "Lunarc") {
        let repeating = (second ? repeating1.value : repeating2.value);
        let remainder = (repeating > 0 ? (repeating & 1 == 1 ? "Odd" : "Even") : "None");
        if (move.mr2 == "Ranged Defense" && remainder == "Even") {
            multi *= 1.3;
            stuffUsed.ability2 = ability2;
        } else if (move.mr2 == "Melee Defense" && remainder == "Odd") {
            multi *= 1.3;
            stuffUsed.ability2 = ability2;
        } 
    }
    ...
    if ((rain.checked && loom2.types.includes("Ice") && move.mr2 == "Ranged Defense") || (sandstorm.checked && loom2.types.includes("Crystal")) && move.mr2 == "Melee Defense") {
        multi *= 1.25;
    }
    ...
    else if ((isStab(types1, { type: tempType }) || ability1 == "Altertype") && !pylons) {
        multi *= 1.5;
        if (ability1 == "Altertype") stuffUsed.ability1 = ability1;
    }
    ...
    if (types[types2.primary.toLowerCase()].immunities.includes(tempType.toLowerCase()) && !((((ability1 == "Grounded" && immuneBoostCheck1) || itemB == "Heavy Blanket") && loom2.types.includes("Air"))) && !(ability1 == "Barkenstein" && (tempType == "Melee" || tempType == "Basic") && types2.primary == "Spirit") && !(move.name == "Breakneck Blitz" && types2.primary == "Earth") && !((loom2.types.includes("Metal") || loom2.types.includes("Crystal")) && tempType == "Poison" && acidRain.checked)) {
    ...
    if (types2.secondary != "None" && types[types2.secondary.toLowerCase()].immunities.includes(tempType.toLowerCase()) && !((((ability1 == "Grounded" && immuneBoostCheck1) || itemB == "Heavy Blanket") && loom2.types.includes("Air"))) && !(ability1 == "Barkenstein" && (tempType == "Melee" || tempType == "Basic") && types2.secondary == "Spirit") && !(move.name == "Breakneck Blitz" && types2.secondary == "Earth") && !((loom2.types.includes("Metal") || loom2.types.includes("Crystal")) && tempType == "Poison" && acidRain.checked)) {
        /*if (ability2 == "Spiteful") {
            multi *= 2;
            spiteWeak *= 2;
            stuffUsed.ability2 = ability2;
    ...
    if ((loom2.types.includes("Metal") || loom2.types.includes("Crystal")) && tempType == "Poison" && acidRain) {
        multi *= 0.5;
    }    
    ...
    /*if (loom2.types.includes("Food") && chocolateRain.checked && effectiveness > 1) {
        multi *= 0.75;
    ...
    if ((ability1 == "Nullify") ||
        (ability1 == "Annihilation") ||
        (ability1 == "Grounded" && immuneBoostCheck1 && (ability2 == "Levitate" || ability2 == "Sky Deity"))) {
    ...
    if (ability2 == "Eruption" && immuneBoostCheck2 && tempType == "Ice") {
        multi *= 0;
        stuffUsed.ability2 = ability2;
    }
    ...
    if (ability1 == "Survival Instinct") {
        let rawDefense = normieStats1.def;
        rawDefense = Math.max(normieStats1.def,normieStats1.defR);
    ...
    if (ability2 == "Survival Instinct") {
        let rawDefense = normieStats2.def;
        rawDefense = Math.max(normieStats2.def,normieStats2.defR);
    ...
    if ((ability2 == "King's Edict" && stats2.hpPercent < 50) ||
        (ability2 == "Foul Welcome" && immuneBoostCheck2) ||
        (ability2 == "Fixation" && immuneBoostCheck2)) {
        multi *= 0.8;
        stuffUsed.ability2 = ability2;
    }
    ...
    if ((effectiveness > 1 && (ability2 == "Unbreakable" || ability2 == "Salt Safeguard"))) {
        multi *= 2/3;
        stuffUsed.ability2 = ability2;
    }
    ...
    if ((ability2 == "Warden" && !pylon && btl1 && withoutSlapDown && !foulHit) ||
        (ability2 == "Home Ground" && stat1 == "marked")) {
        multi *= 0.5;
        stuffUsed.ability2 = ability2;
    }
    ...
    if (move.name == "Cauterizing Smash" && stat2 == "burned" && withoutSlapDown && !foulHit) {
        multi *= 3;
        stuffUsed.extra1 += " (" + tempPower * 3 + " BP)";
    }
...
function adjustHP(loom1, loom2, move, hp1, hp2, item, ability, status, second = false, counter, OHKO, onlyIncludeIceTrap = false) {
    ...
    let charcoil = charcoil2.checked;
    let hold = hold2.checked;
    ...
    let sapHeal = (garden.checked ? 12.5/100 : 8/100);
    
    if (!loom2.types.includes("Plant") && sap.defender == true && ability != "Direct Combatant" && ability != "Impervious" && ability != "Nullcoat") {
    ...
    if (ability != "Direct Combatant" && ability != "Impervious" && ability != "Nullcoat") {
        ...
        if (charcoil) {
            ...
            hazardString += "charcoil damage and ";
        }

        if (hold) {
            newHP += Math.floor(hp1 * 1 / 16);
            hazardString += "infernal hold damage and ";
        }
    ...
    if (!OHKO && ability != "Metabolize") {
        ...
        if (item == "Dark Chocolate") {
            ...
            } else if (ability != "Direct Combatant" && ability != "Impervious" && ability != "Nullcoat") {
                newHP += Math.floor(hp1 * 1 / 16);
    ...
    if (ability != "Direct Combatant" && ability != "Impervious" && ability != "Nullcoat") {
        if (status == "cursed" && calamity.checked) {
            newHP += Math.floor(hp1 * 1 / 8);
            hazardString += "calamity damage and ";
        }
        if (status == "burned" && !loom2.types.includes("Fire") && ability != "Aqua Body" && move.name != "Cauterizing Smash" && !aquagel && ability != "Steeped Spirit" && ability != "Fireproof Armor" && ability != "Extinguisher" && ability != "Hard Candy") {
            if (otherAbility == "First Degree Burns") newHP += Math.floor (hp1 * 1 / 6);
            if (ability == "Crispy") newHP += Math.floor (hp1 * 1 / 8);
            newHP += Math.floor(hp1 * 1 / 16);
...
function checkIceTrap(move, l, u, hp, hpPercent, item, ability, ability2, stat1, stat2, mon1, mon2) {
    ...
    if (ability != "Metabolize" && (move.drain || (ability == "Leech" && move.type == "Insect") || (ability == "Poison Substance" && move.type == "Poison") || (ability == "Vampire" && move.type == "Dark") || (move.name == "Chaotic Bolt" && stat2 == "marked") ||
       ...
       (ability == "Thunder Gut" && hpPercent < 50 && move.type == "Spark") || ((mon1.name == "Velace" || mon1.name == "Curixen") && item == "Siphon Egg" && (move.type == "Fire" || move.type == "Light")) || (ability == "Umbral Hunger" && (move.type == "Dark" || move.type == "Crystal") && stat2 != "healthy"))) {
        ...
        if ((ability == "The Fungus" && move.mr == "Magic") || (ability == "Soul Fortification" && hpPercent < 50 && move.mr == "Melee") || (ability == "Thunder Gut" && hpPercent < 50 && move.type == "Spark")) {
            ...
            if (!drain) drain = 1/3;
                   ...
                   (ability == "Proboscus" && move.mr == "Melee") || ((mon1.name == "Velace" || mon1.name == "Curixen") && item == "Siphon Egg" && (move.type == "Fire" || move.type == "Light")) || (ability == "Umbral Hunger" && (move.type == "Dark" || move.type == "Crystal") && stat2 != "healthy")) {
            if (!drain) drain = 1/2;
            else drain += 1/2;
        }
    ...
    if (move.recoil || (icyFloor.checked)) {
        if (ability == "Tenacious" || ability == "Direct Combatant" || ability == "Impervious" || ability == "Nullcoat") return "";
        let recoil = move.recoil;
        if (icyFloor.checked && stat1 == "frozen" && !(mon1.types.includes("Ice"))) {
            if (!recoil) recoil = 1/3;
            else recoil += 1/3;
        }
        let recoilL = (Math.max(Math.floor(l * recoil), 1));
        let recoilU = Math.max(Math.floor(u * recoil), 1);
        if (recoil) return " (" + (recoilL / hp * 100).toFixed(1) + " - " + (recoilU / hp * 100).toFixed(1) + "% recoil damage)";
        else return "";
    }
    /*if (move.name == "Flail") {
